// 简单的Foxglove数据加载器测试用例
// 这个测试文件用于验证RecordDataLoader的基本功能，不依赖TypeScript编译
...
const mockExtensionContext = {
  registerDataLoader: (config) => {
    console.log('数据加载器注册:', {
      ...
      wasmUrl: config.wasmUrl ? 'WASM URL已设置' : 'WASM URL未设置'
    });
    return {
      type: config.type,
      supportedFileType: config.supportedFileType,
...
async function runTests() {
  console.log('=== Foxglove Record Data Loader 简单测试 ===');
  ...
  try {
    ...
    const mockConfig = {
      ...
      supportedFileType: ".record"
    };
    
    const result = mockExtensionContext.registerDataLoader(mockConfig);
    console.log('✓ 插件注册模拟测试通过');
    ...
    const sectionType = dataView.getUint32(0, true);
    const sectionLength = dataView.getUint32(4, true);
    console.log(`✓ Section类型: ${sectionType}`);
    console.log(`✓ Section长度: ${sectionLength}`);
